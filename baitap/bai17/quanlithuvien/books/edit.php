...
$items = $stmt->fetch();
 ...
 if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(empty($err)){
        $sql = "UPDATE books SET `name` = '$Name', `category_id` = $category_id, `price` = $price  WHERE id='$id'";
        $conn->query($sql);
    ...
    <div class="container">
        <form method="post" action="">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="name" id="" class="form-control" placeholder="Tên sách" value="<?php echo $items->name; ?>">
                ...
                <span><?php if (isset($err['name'])) {
                 ...
                 <select name="category" class="form-control" id="">
                    ...
                    <option <?=$row->id == $items->category_id ? "selected" : " " ?> value="<?=$row->id;?>"><?=$row->name_category;?></option>
                    <?php } ?>
                </select><br>
